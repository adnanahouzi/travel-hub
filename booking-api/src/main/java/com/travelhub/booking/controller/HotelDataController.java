...
public class HotelDataController {
    ...
    @Operation(
            ...
            description = "Search for places (cities, regions, etc.) by text query. This can be used to get place IDs for hotel searches."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successfully retrieved matching places",
                    content = @Content(schema = @Schema(implementation = PlaceSearchResponseDto.class))
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    public ResponseEntity<PlaceSearchResponseDto> searchPlaces(
            @Parameter(description = "Place search request with text query and optional parameters")
            @RequestBody PlaceSearchRequestDto request
    ) {
    ...
    @Operation(
            ...
            description = "Get reviews for a specific hotel, optionally with sentiment analysis."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Successfully retrieved hotel reviews",
                    content = @Content(schema = @Schema(implementation = HotelReviewsResponseDto.class))
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "Invalid request parameters"
    ...
    public ResponseEntity<HotelReviewsResponseDto> getHotelReviews(
            ...
            @Parameter(description = "Max number of reviews to return", example = "10")
            @RequestParam(required = false) Integer limit,
            @Parameter(description = "Offset for pagination", example = "0")
            @RequestParam(required = false) Integer offset,
            @Parameter(description = "Request timeout in milliseconds", example = "5000")
            @RequestParam(required = false) Integer timeout,
            @Parameter(description = "Include sentiment analysis", example = "true")
            @RequestParam(required = false) Boolean getSentiment
    ) {
        logger.info("Received hotel reviews request - hotelId: {}, limit: {}, getSentiment: {}", 
                hotelId, limit, getSentiment);
        
        HotelReviewsResponseDto response = hotelDataService.getHotelReviews(hotelId, limit, offset, timeout, getSentiment);
