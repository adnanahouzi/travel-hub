...
public class RateController {
        ...
        @PostMapping("/search")
        @Operation(summary = "Search hotel rates", description = "Search for available hotel rates based on various criteria including location, dates, and guest information")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved hotel rates", content = @Content(schema = @Schema(implementation = RateSearchResponseDto.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request parameters"),
                        @ApiResponse(responseCode = "500", description = "Internal server error")
        })
        public ResponseEntity<RateSearchResponseDto> searchRates(
                        @Parameter(description = "Hotel search criteria including dates, location, and guest information") @RequestBody RateSearchRequestDto request) {
        ...
        @PostMapping("/{hotelId}")
        @Operation(summary = "Get rates for a specific hotel", description = "Retrieves detailed hotel information combined with available rates. "
                        +
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved hotel details with rates", content = @Content(schema = @Schema(implementation = HotelRateResponseDto.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request parameters"),
        ...
        public ResponseEntity<HotelRateResponseDto> getHotelRates(
                        ...
                        @Parameter(description = "Rate search criteria including dates, occupancy, and guest information") @RequestBody HotelRateRequestDto request) {
                ...
                logger.info("Hotel rate request completed - hotelId: {}, grouped offers found: {}",
                                hotelId, response.getGroupedRates() != null ? response.getGroupedRates().size() : 0);
